...
cat <<EOF
Usage:
   ${program} all|pfsense -s3 <bucket_name> -region <region>

   Example:
      ${program} -o pfsense -s3 demo_bucket -region us-east-1
...
build ()
{
   ...
   if [ "${1}" == "all" ]; then
      ...
      packer build -force config/packer-pfsense.json
      find ./output >> ${log}
   else
      packer build -force config/packer-${1}.json
      ls -l output/${1}/* >> ${log}
   fi
   addLog "Done."
}
